...
fourX_filtered = full_fft_x1(1:length(H_full_x1)) .* H_full_x1';
fourY_filtered = full_fft_x2(1:length(H_full_x2)) .* H_full_x2';

% Convert back to time domain (inverse FFT)
dx1dt_filtered = ifft([fourX_filtered; zeros(length(full_fft_x1) - length(fourX_filtered), 1)], 'symmetric');
dx2dt_filtered = ifft([fourY_filtered; zeros(length(full_fft_x2) - length(fourY_filtered), 1)], 'symmetric');
